...
export default function App() {
  ...
  const [authCallbackDone, setAuthCallbackDone] = useState(false)

  // Detect if we're running standalone (not in an iframe)
  ...
  useEffect(() => {
    ...
    if (code) {
      ...
      exchangeCode(code).then(() => {
        ...
        sendStateUpdate()
        // If we're in a standalone tab (not iframe), try to close it
        if (!isInIframe) {
          setAuthCallbackDone(true)
          window.close()
        }
      }).catch((err) => {
  ...
  // Listen for cross-tab localStorage changes to detect auth from the OAuth callback tab
  useEffect(() => {
  ...
  // --- Auth callback complete (standalone tab that didn't auto-close) ---
  if (authCallbackDone) {
    return (
      <div style={styles.container}>
        <div style={styles.authCard}>
          ...
          <div style={styles.authTitle}>Connected!</div>
          <div style={styles.authSubtext}>
            You can close this tab and return to ChatBridge.
          </div>
        </div>
      </div>
    )
